...
[Lagoon](https://lagoon.sh/) solves what developers are dreaming about: A system that allows developers to locally develop their code and their services with Docker and run the exact same system in production. The same Docker images, the same service configurations and the same code.

This is currently an _beta_ level integration that has the following _serious caveats_:

* This _does not_ support Lagoon's `python`, `mongodb`, or `nodejs` containers yet
...
As per the general Lando [Proxy](./../config/proxy.md#usage) instructions, it is possible to configure custom domains (i.e. `myapp.lndo.site` or `foo.bar.xyz`) for your site.  However, as the default Lagoon base images are built slightly differently to the default Lando ones, you need to [specify the port](./../config/proxy.md#routing-to-a-different-port) to route the request to (usually 8080 for nginx):
